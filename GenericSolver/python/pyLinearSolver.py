...
class LCGsolver(Solver):
    ...
    def run(self, problem, verbose=False, restart=False):
        ...
        while True:
            ...
            if obj1 >= obj0:
                msg = "Objective function didn't reduce, will terminate solver: obj_new=%.2e obj_current=%.2e" % (obj1, obj0)
                if verbose:
                    print(msg)
                # Writing on log file
                if self.logger:
...
class SymLCGsolver(Solver):
    ...
    def run(self, problem, verbose=False, restart=False):
        ...
        if not restart:
            msg = "PRECONDITIONED " if precond else ""
            msg += "LINEAR %s SOLVER FOR SYMMETRIC MATRIX" % ("STEEPEST-DESCENT" if self.steepest else "CONJUGATE GRADIENT")
            if verbose:
                print(msg)
            if self.logger:
                self.logger.addToLog(msg + " log file")
