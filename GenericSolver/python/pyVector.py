...
try:
    ...
    import GenericSolver.python.sep_util as sep_util
...
class superVector(vector):
    ...
    def checkSame(self, vecs_in):
        ...
        checkspace = np.asarray([self.vecs[idx].checkSame(vecs_in.vecs[idx]) for idx in range(self.n)])
...
class vectorOC(vector):
    ...
    def clone(self):
        """
        Function to clone (deep copy) a vector or from a space
        and creating a copy of the associated header file
        """
        ...
        if vec_clone.vecfile is None:
            ...
            # Placing temporary file into datapath folder
            tmp_vec = sep_util.datapath + "clone_tmp_vector" + str(
                int(time.time() * 1000000)) + ".H"
            axis_file = ""
            ...
            # Placing temporary file into datapath folder
            tmp_vec = sep_util.datapath + tmp_vec + "_clone_" + str(
                int(time.time() * 1000000)) + ".H"
            tmp_bin = tmp_vec + "@"
    ...
    def cloneSpace(self):
        ...
        (vector without actual vector binary file by using None values)
        """
    ...
    def isDifferent(self, vec2):
        ...
        # Checking whether the input is a vector or not
        if not isinstance(vec2, vectorOC):
            raise TypeError("Provided input vector have to be a vectorOC!")
