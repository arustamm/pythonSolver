...
class DaskClient:
	...
	def __init__(self, **kwargs):
		"""
		   ...
		   :param hostnames : - list; list of strings containing the host names or IP addresses of the machines that the user wants to use in their cluster/client (First hostname will be running the scheduler!) [None]
		...
		if hostnames:
			...
			cmd = ["ssh"] + [hostnames[0]] + ["dask-scheduler"] + ["--scheduler-file"] + [scheduler_file] + [
				"--port"] + [
					  self.port]
			...
			for hostname in hostnames:
				...
				worker_ips.append(
					subprocess.check_output(["ssh"] + [hostname] + ["hostname -I"] + ["| awk '{print $1}'"]).rstrip().decode("utf-8"))
