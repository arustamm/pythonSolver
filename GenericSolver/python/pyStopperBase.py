...
class BasicStopper(pyStopper.Stopper):
	...
	def __init__(self,niter=0,maxfevals=0,maxhours=0.0,tolr=1.0e-18,tolg=1.0e-18,tolobj=None,tolobjrel=None,toleta=None,tolobjchng=None,logger=None):
		"""
	...
	def run(self,prblm,iter,initial_obj_value=None,verbose=True):
		...
		if(self.tolobj!=None):
			if(obj < self.tolobj):
				...
				if(self.logger): self.logger.addToLog(msg)
				return stop
		if(self.tolobjrel!=None and initial_obj_value!=None and initial_obj_value != 0.0):
		...
		if(self.toleta!=None):
			...
			if(res_norm < self.toleta*data_norm):
				...
				if(self.logger): self.logger.addToLog(msg)
				return stop
		if(self.tolobjchng != None and initial_obj_value != None and initial_obj_value!=0.0):
			#Saving current scaled objective function value
			self.obj_pts.append(obj/initial_obj_value)
			if(iter >= self.ave_pts):
				...
				if(objchng < self.tolobjchng):
					stop = True
					msg =  "Terminate: tolerance on the change of the relative objective function value (i.e., phi(m_i) - phi(m_i-1)/ phi(m_0)) of %s reached (computed using %s points), change value %s"%(self.tolobjchng,self.ave_pts,objchng)
					if(verbose): print(msg)
					if(self.logger): self.logger.addToLog(msg)
					return stop
		return stop
